...
def test_e2e_cooperative_flow(auth_token):
    ...
    create_resp = requests.post(
        f"{BASE_URL}/cooperatives", json=coop_data, headers=headers
    )
    ...
    try:
        analysis_resp = requests.post(
            f"{BASE_URL}/peru/cooperatives/{coop_id}/analyze", headers=headers
        )
...
def test_e2e_roaster_flow(auth_token):
    ...
    create_resp = requests.post(
        f"{BASE_URL}/roasters", json=roaster_data, headers=headers
    )
    assert create_resp.status_code == 201
...
def test_e2e_margin_calculation(auth_token):
    ...
    coop_resp = requests.post(
        ...
        json={"name": "Margin Test Coop", "region": "Junín", "annual_volume_kg": 20000},
        headers=headers,
    )
...
def test_ml_predictions_available(auth_token):
    ...
    try:
        ml_resp = requests.post(
            f"{BASE_URL}/ml/predict-freight", json=freight_payload, headers=headers
        )
...
def test_e2e_shipment_flow(auth_token):
    ...
    create_resp = requests.post(
        f"{BASE_URL}/shipments", json=shipment_data, headers=headers
    )
    assert create_resp.status_code == 201
    ...
    update_data = {"current_location": "Panama Canal", "status": "in_transit"}
    update_resp = requests.patch(
        f"{BASE_URL}/shipments/{shipment_id}", json=update_data, headers=headers
    )
...
def test_frontend_accessibility():
    ...
    try:
        ...
        pytest.skip(
            "Frontend not accessible (may not be running or configured differently)"
        )
