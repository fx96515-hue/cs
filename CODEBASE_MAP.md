...
│       └── middleware.ts           ← Route-Guard (Auth, passthrough bis httpOnly-Cookie)
...
| `apps/web/middleware.ts` | Next.js Route-Guard. Matcher: `/dashboard`, `/roasters/:path*`, `/cooperatives/:path*`. Wartet auf httpOnly-Cookie (Roadbook Schritt 1). |
...
| Middleware nur passthrough | `middleware.ts` | Aktivieren nach Schritt 1 |
