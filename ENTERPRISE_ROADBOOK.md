...
| Route-Guard Middleware | `proxy.ts` | Passthrough — aktivieren nach Schritt 1 |
...
Die `proxy.ts` ist bereits vorhanden und schützt alle Routen. Sie greift sobald der Token als Cookie kommt (Schritt 1). **Keine weiteren Änderungen nötig.**
...
- **Sicherheit:** `ApiError`-Klasse, 401-Auto-Refresh, `proxy.ts` Route-Guard (wartet auf Cookie)
