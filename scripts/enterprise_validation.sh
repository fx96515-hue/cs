...
# We intentionally avoid set -e so all checks run and aggregate failures.
set -uo pipefail
...
if [[ $TEST_EXIT_CODE -eq 0 ]]; then
    ...
    if [[ -n "$COVERAGE" ]]; then
        if [[ "${COVERAGE%.*}" -ge 57 ]]; then
            ...
            print_warning "Code coverage: ${COVERAGE}% (below target 57%)"
        fi
    fi
else
...
if [[ ! -f "$ENV_EXAMPLE" ]]; then
    ...
    if [[ ${#MISSING_VARS[@]} -eq 0 ]]; then
        print_success "All required variables present in .env.example"
    else
